...
# 01
    def __init__(self):
        self.stock = {'name':stock.player_1.name[17],'code':'s018','price':stock.player_1.start[17],'change':0}
        self.msg = ''
...
# 03
    def buyStock(self):
        if stock.player_1.player['cash'] >= self.stock['price']:
            stock.player_1.player['s018'][1] += self.stock['price']
            stock.player_1.player['s018'][0] += 1
...
# 04
    def sellStock(self):
        if stock.player_1.player['s018'][0] >= 1:
            stock.player_1.player['s018'][1] -= stock.player_1.player['s018'][1]/stock.player_1.player['s018'][0]
            stock.player_1.player['s018'][0] -= 1
            stock.player_1.player['cash'] += self.stock['price']
