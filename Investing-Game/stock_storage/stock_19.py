...
# 01
    def __init__(self):
        self.stock = {'name':stock.player_1.name[18],'code':'s019','price':stock.player_1.start[18],'change':0}
        self.msg = ''
...
# 03
    def buyStock(self):
        if stock.player_1.player['cash'] >= self.stock['price']:
            stock.player_1.player['s019'][1] += self.stock['price']
            stock.player_1.player['s019'][0] += 1
...
# 04
    def sellStock(self):
        if stock.player_1.player['s019'][0] >= 1:
            stock.player_1.player['s019'][1] -= stock.player_1.player['s019'][1]/stock.player_1.player['s019'][0]
            stock.player_1.player['s019'][0] -= 1
            stock.player_1.player['cash'] += self.stock['price']
