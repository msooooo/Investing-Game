...
import stock_storage.stock_13 as s013
import stock_storage.stock_14 as s014
import stock_storage.stock_15 as s015
import stock_storage.stock_16 as s016
import stock_storage.stock_17 as s017
import stock_storage.stock_18 as s018
import stock_storage.stock_19 as s019
import stock_storage.stock_20 as s020
import virtual_time
...
# ------------------------------------------------------------------------------------------------------------------ #
        ...
        for code in self.code:
            ...
            if share == 0:
                ...
                average = round(stock.player_1.player[code][1]/share,1)
            ...
            if stock.player_1.player['profits'][0+c] >= 0 and stock.player_1.player[code][0] >= 1:
                self.playerStockTable.append((name,share,average,f'+{profit}',f'+{float(profit_percentage)} %'))
            elif stock.player_1.player[code][0] >= 1:
                self.playerStockTable.append((name,share,average,f'-{abs(profit)}',f'-{abs(profit_percentage)} %'))

        self.playerStock = tkinter.ttk.Treeview(master,columns=('Stock','Share','Average','Profit','Profit_Percentage'),show='headings')

        self.playerStock.column("Stock",width=115)
        self.playerStock.column("Share",width=20)
        self.playerStock.column("Average",width=40)
        ...
        self.playerStock.heading("Average",text="Average")
...
# 03
    def main_stat(self):
        ...
        if vt.getTimeString() == stock.player_1.marketStatus[2]:
            self.candleTurn = 0
            self.candleDay += 1
            self.s018.initialPrice = self.s018.priceHistory[-1]
            self.s019.initialPrice = self.s019.priceHistory[-1]
            self.s020.initialPrice = self.s020.priceHistory[-1]
            self.priceRecord()
        ...
        # ETF
        self.s018.ratio = 1 + ((self.s001.stock['change'] + self.s002.stock['change'] + self.s003.stock['change'] + self.s004.stock['change'] + self.s005.stock['change'] + self.s006.stock['change'] + self.s007.stock['change'] + self.s008.stock['change'] + self.s009.stock['change'] + self.s010.stock['change'] + self.s011.stock['change'] + self.s012.stock['change'] + self.s013.stock['change'] + self.s014.stock['change'] + self.s015.stock['change'] + self.s016.stock['change'] + self.s017.stock['change'])/1700)
        self.s019.ratio = 1 - ((self.s001.stock['change'] + self.s002.stock['change'] + self.s003.stock['change'] + self.s004.stock['change'] + self.s005.stock['change'] + self.s006.stock['change'] + self.s007.stock['change'] + self.s008.stock['change'] + self.s009.stock['change'] + self.s010.stock['change'] + self.s011.stock['change'] + self.s012.stock['change'] + self.s013.stock['change'] + self.s014.stock['change'] + self.s015.stock['change'] + self.s016.stock['change'] + self.s017.stock['change'])/1700)
        self.s020.ratio = 1 + ((self.s001.stock['change'] + self.s003.stock['change'] + self.s009.stock['change'] + self.s011.stock['change'] + self.s014.stock['change'] + self.s015.stock['change'] + self.s016.stock['change'])/700)
